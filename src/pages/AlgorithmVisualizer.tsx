...
function* bubbleSortGen(arr: number[]) {
  ...
  yield { array: [...a], comparing: [] as number[], sorted: a.map((_, i) => i) }
}
...
function linearSearchGen(arr: number[], target: number) {
  function* gen() {
    for (let i = 0; i < arr.length; i++) {
      yield { array: [...arr], comparing: [i], found: arr[i] === target ? i : -1 }
...
function binarySearchGen(arr: number[], target: number) {
  ...
  function* gen() {
    ...
    while (lo <= hi) {
      ...
      yield { array: sorted, comparing: [mid], lo, hi, found: sorted[mid] === target ? mid : -1 }
...
export default function AlgorithmVisualizer() {
  ...
  const [inputData, setInputData] = useState<number[]>([38, 27, 43, 3, 9, 82, 10])
  const [currentStep, setCurrentStep] = useState<Step>({ array: [38, 27, 43, 3, 9, 82, 10], comparing: [] })
  ...
  const buildGenerator = useCallback((data: number[], algo: string): Generator<Step> => {
    const target = Math.max(...data)
    switch (algo) {
      case 'bubble-sort': return bubbleSortGen(data) as Generator<Step>
      case 'selection-sort': return selectionSortGen(data) as Generator<Step>
      case 'insertion-sort': return insertionSortGen(data) as Generator<Step>
      case 'linear-search': return linearSearchGen(data, target) as Generator<Step>
      case 'binary-search': return binarySearchGen(data, target) as Generator<Step>
      default: return bubbleSortGen(data) as Generator<Step>
    }
  }, [])
  ...
  useEffect(() => { reset() }, [selectedAlgorithm, inputData])
  ...
  const getBarColor = (index: number) => {
    ...
    if (currentStep.found !== undefined && currentStep.found === -1 && isDone) return 'bg-red-500/60'
    if (currentStep.sorted?.includes(index)) return 'bg-green-500'
    if (currentStep.comparing.includes(index)) return 'bg-yellow-400'
    return 'bg-blue-500'
  }

  const isSortAlgo = ['bubble-sort', 'selection-sort', 'insertion-sort', 'merge-sort', 'quick-sort'].includes(selectedAlgorithm)
    ...
    <div className="space-y-6">
      <div className="grid lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          {/* Controls */}
          <div className="flex gap-2">
            <button onClick={() => { if (isDone) reset(); setIsRunning(r => !r) }}
              ...
              {isRunning ? <><Pause className="w-4 h-4" /> Pause</> : isDone ? <><RotateCcw className="w-4 h-4" /> Restart</> : <><Play className="w-4 h-4" /> Start</>}
            </button>
            <button onClick={reset} className="bg-slate-700 hover:bg-slate-600 px-4 py-3 rounded-lg transition">
          ...
          <div className="bg-slate-800 p-4 rounded-lg border border-slate-700 text-sm space-y-1">
            <div className="flex justify-between"><span className="text-slate-400">Steps</span><span className="text-white font-mono">{stepCount}</span></div>
            <div className="flex justify-between"><span className="text-slate-400">Elements</span><span className="text-white font-mono">{currentStep.array.length}</span></div>
            <div className="flex justify-between"><span className="text-slate-400">Status</span>
              <span className={isDone ? 'text-green-400' : isRunning ? 'text-yellow-400' : 'text-slate-400'}>
        ...
        <div className="lg:col-span-2">
          <div className="bg-slate-800 rounded-lg border border-slate-700 p-6" style={{ minHeight: '420px' }}>
            <h3 className="text-sm font-semibold text-slate-400 mb-4 uppercase tracking-wide">
              {selectedAlgorithm.replace(/-/g, ' ')} — {isSortAlgo ? 'Sorting' : 'Searching'}
            </h3>
            ...
            <div className="flex gap-4 mt-4 text-xs text-slate-400 flex-wrap">
              <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-blue-500 inline-block" /> Default</span>
              <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-yellow-400 inline-block" /> Comparing</span>
              <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-green-500 inline-block" /> {isSortAlgo ? 'Sorted' : 'Found'}</span>
            </div>

            {isDone && (
              <div className="mt-4 p-3 bg-green-900/20 border border-green-600 rounded text-green-400 text-sm text-center">
                ✅ {isSortAlgo ? 'Array sorted successfully!' : `Search complete in ${stepCount} steps`}
              </div>
            )}
          </div>
        </div>
